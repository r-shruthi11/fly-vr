...
%to do: leave for long time, see if can optimize vfov; optimize the missing
%two params; fix: Y or -Y in XYZ map
...
if strcmpi(IsOK,'N')
   ...
   for nMaskFile = 1:size(MASKfiles,1)
      ...
      if isempty(strfind(MASK_filename,filename))
         delete(filename)
      end
   end
   ...
   for nSnapFile = 1:size(SNAPfiles,1)
      ...
      if isempty(strfind(SnapFile,filename))
         delete(filename)
      end
   end
