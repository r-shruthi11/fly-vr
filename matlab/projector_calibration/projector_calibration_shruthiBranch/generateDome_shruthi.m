...
% r = mirror radius = 30.9mm
% xOm,yOm,zOM = cooridnates of mirror center w.r.t fly
% #### this could be wrong nomenclature by AJC
% xP1m, yP1m, zP1m = coordinates of projector center w.r.t screen center.
...
% 1. How to set figure limits?
% 2 Which figure limits to use?
...
XProjectorMax = 608/684;%/608; %SR
XProjectorMin = -608/684;%/608; %SR
YProjectorMax = 1; %SR
YProjectorMin = -1;
...
%         end
        ...
        % xOm,yOm,zOM = cooridnates of mirror center w.r.t fly
        % #### this could be wrong nomenclature by AJC
        % xP1o, yP1o, zP1o = coordinates of projector center w.r.t screen center.

        [HorizPx,VerticPx] = getHorizVertFromAngles(elevat,azim,radius,Rs,r, xsm,ysm,zsm, ...
                                                        xOs,yOs,zOs,xP1o,yP1o,zP1o,x0,y0,x1,y1);
...
%%% Edited: SR
plot(XProjectorMin, YProjectorMin,'w*', 'MarkerSize', 14);
plot(XProjectorMin, YProjectorMax,'w*', 'MarkerSize', 14);
plot(XProjectorMax, YProjectorMin,'w*', 'MarkerSize', 14);
plot(XProjectorMax, YProjectorMax,'w*', 'MarkerSize', 14);
...
writeCalibration(xVals,yVals,azimAll,elevAll,Xscale,Yscale, x2,y2);%SR
...
function [HorizPx,VerticPx] = getHorizVertFromAngles(elevat,azim,radius,Rs,r, xsm,ysm,zsm, ...
    ...
    cospsi= xP1o/aab; %%edited : SR
    sinpsi= zP1o/aab; %%edited : SR
    
%     sinpsi= xP1o/aab; %%old: wrong
%     cospsi= zP1o/aab; %%old: wrong
    ...
    % 2quart-
    % what is the angle alpha between the plane OP1P2 and the plane Ox'z'?
...
% edited SR - make these parameters settable from GUI
    VerticPx= y0*sinphi*cosalpha + y1; %SR
    HorizPx= x0*sinphi*sinalpha + x1; %SR

end



function writeCalibration(xVals,yVals,azimAll,elevAll,Xscale,Yscale, x2, y2)%SR
    % file format: http://paulbourke.net/dome/warpingfisheye/
    f = fopen('local_test_Max_calibratedBallImage.data','w');
    fprintf(f,'2\n');
...
%             fprintf(f,'%1.6f %1.6f %1.6f %1.6f %1.6f\n', ...
%                 ((xVals(i,jj) - Xscale(1))./(Xscale(2) - Xscale(1)) *2)-1, ((yVals(i,jj) - Yscale(1))./(Yscale(2) - Yscale(1)) *2)-1,...
%                 (len+1)/2,-elevAll(i,jj)/pi, ...
%                 1);
%             
            
            % SR try rescaling:
%             fprintf(f,'%1.6f %1.6f %1.6f %1.6f %1.6f\n', ...
%                 ((xVals(i,jj) - xVmin)./(xVmax-xVmin))*(Xscale(2)-Xscale(1)) + Xscale(1), ((yVals(i,jj) - yVmin)./(yVmax-yVmin))*(Yscale(2)-Yscale(1)) + Yscale(1),...
%                 (len+1)/2,-elevAll(i,jj)/pi, ...
%                 1);
            % SR try rescaling: correct attempt
            fprintf(f,'%1.6f %1.6f %1.6f %1.6f %1.6f\n', ...
                xVals(i,jj), yVals(i,jj),...
                (len+1)/2,-elevAll(i,jj)/pi, ...
                1);
            
            
            % edit SR:
            xVals_scaled(i,jj) = ((xVals(i,jj) - xVmin)./(xVmax-xVmin))*(Xscale(2)-Xscale(1)) + Xscale(1);
            yVals_scaled(i, jj) = ((yVals(i,jj) - yVmin)./(yVmax-yVmin))*(Yscale(2)-Yscale(1)) + Yscale(1);

            %             SR attempt 1:
%             fprintf(f,'%1.6f %1.6f %1.6f %1.6f %1.6f\n', ...
%                 ((xVals(i,jj)-x2)./(Xscale(2) - Xscale(1)) *2)-1, ((yVals(i,jj)-y2)./(Yscale(2) - Yscale(1)) *2)-1,...
%                 (len+1)/2,-elevAll(i,jj)/pi, ...
%                 1);
